# Claude Code Project Instructions — ts-rs-kotlin

## Project Overview

This is **ts-rs-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`ts-rs`](https://crates.io/crates/ts-rs).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/ts-rs/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/ts-rs>
